...
class EditDistance {
    ...
    Integer editDistanceDP(String pattern, String text){
        ...
        return editDistanceDPChar(pattern, text, pattern.length(), text.length(), cost, parent);
    }
    ...
    private Integer editDistanceDPChar(String pattern, String text, int p, int t, int[][] cost, int[][] parent) {
        ...
        int insertCost = editDistanceDPChar(pattern, text, p-1, t, cost, parent) + 1;
        int deleteCost = editDistanceDPChar(pattern, text, p, t-1, cost, parent) + 1;
        int finalCost = matchCost;
        if(finalCost > insertCost){
            finalCost = insertCost;
        }
    ...
    @ParameterizedTest
    @CsvSource({"abbccddeeffgg,abbccdeeeffff, 3","catpillarrrrlq, patpillaerrols, 4", "hal, hal, 0", "advise, advice, 1", "he, she, 1", "a, b, 1"})
